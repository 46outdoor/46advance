...
// NOTE: `drive.metadata.readonly` was previously requested here so importDriveFolder could
// enumerate the library folder (`drive.file` alone can't list files the app didn't create).
// Google classifies it as a RESTRICTED scope (confirmed in the console's Data Access page),
// which forces the heavyweight verification track (security assessment). The import now
...
export const googleAuthUrl = onCall({ secrets: OAUTH_SECRETS }, async (request) => {
  ...
  // `expiresAt` lets an optional native Firestore TTL policy auto-expire abandoned states; the
  // retention sweep prunes them by `createdAt` regardless (WS-H).
  await stateRef.set({
