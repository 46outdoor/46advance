...
const CELL: Record<SectionStatus, { className: string; glyph: string; label: string }> = {
  ...
  in_progress: {
    className: 'bg-status-progress/25 text-status-progress',
    glyph: '◐',
    label: 'In progress',
  },
  complete: {
    className: 'bg-status-complete/25 text-status-complete',
    glyph: '✓',
    label: 'Complete',
  },
};
...
function StatusCell({ status }: { status: SectionStatus | null }) {
  ...
  return (
    <td
      className={`border border-line/60 text-center text-sm font-semibold ${c.className}`}
      title={c.label}
    >
...
export function TrackerGrid({ eventId, tracker }: { eventId: string; tracker: EventTracker }) {
  if (tracker.rows.length === 0) {
    return (
      <p className="text-sm text-ink-muted">
        No artist advances yet — add stages and advances to populate the tracker.
      </p>
    );
  }
  if (tracker.columns.length === 0) {
    return (
      <p className="text-sm text-ink-muted">
        This event has no departments enabled, so there are no sections to track.
      </p>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr>
            <th className="sticky left-0 z-10 bg-surface px-2 py-2 text-left font-semibold text-brand">
              Advance
            </th>
            {tracker.columns.map((col) => (
              <th
                key={col.id}
                className="border border-line/60 px-2 py-2 text-center text-xs font-semibold text-ink-muted"
              >
                {col.name}
              </th>
            ))}
