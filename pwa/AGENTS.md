...
| Permission checks (global directories) | `canBrowseGlobalDirectories` in `src/lib/rbac/permissions.ts` — the contacts directory + artist document library (`/contacts`, `/documents`): admin ∨ organizer ∨ director ∨ coordinator. Mirrored server-side by the same-named predicate in `functions/src/lib/auth/authorize.ts` and (from PR 2) by the `contacts`/`artistDocuments` read rules. ⚠ Takes NO per-event role — a global collection's rules gate cannot see one, which is why a PM who needs the directory holds `organizer` (`planning/ACCESS_SCOPING_PLAN.md` decision 1). Route guard: `src/components/CapabilityGate.tsx` |
