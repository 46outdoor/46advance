...
| Event contact attachments | `src/features/events/event-contacts-service.ts` (per-event join). ⚠ Detach is **server-only** since 2026-08-20 — `detachContact` calls the `detachEventContact` callable, which refuses while crew-logistics records reference the attachment; the rules deny direct deletes |
