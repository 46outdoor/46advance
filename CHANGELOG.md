...
- **Artist documents:** a **Documents** library — **import an artist-docs Google Drive folder** (per-artist subfolders tag the artist, nested subfolders included), then browse per artist and **classify** each file by type. Files link to Drive (least-privilege metadata-only access; you pick the folder). Each document can also get an **in-app title** (without renaming it in Drive), **notes**, and an **obsolete** flag — none of which touch the Drive file. Admins/organizers manage.
