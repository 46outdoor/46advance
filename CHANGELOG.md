...
- **View artist documents in-app (Drive broker):** approved users get a **View** button on artist documents that opens the file **even without direct Google Drive access** — a dedicated service account (with read access to the docs folder) serves the file through the app, gated by app access (the file must be a known artist document). Techs no longer need to be individually granted Drive access to see riders/plots.
