...
| Newly surfaced routes | **Tracker** for everyone; **Templates** and **Schedule templates** for admins |
| Inline breakpoint | **800px** (`min-[800px]:`) — see rationale below |
| Role policy | Cross-event Contacts/Documents for **organizer or admin**; event-scoped Tracker for everyone |
...
An approved PM, department lead, or tech who is **not** also a global organizer sees:
...
| Tracker | everyone | Its overview uses the same membership-scoped event set |
...
- **Destinations** (Events, Contacts, Documents, Tracker) — day-to-day. Tracker is shown to
  everyone because its data is already membership-scoped and the Events screen already
  links to it for every approved user. Contacts and Documents are cross-event directories,
  so their navigation entries follow the global organizer/admin presentation policy.
...
type NavVisibility = 'all' | 'cross-event' | 'admin';
...
- `visibility` resolves once through a pure nav-specific function: `all`; `cross-event`
...
as `PERSONAS.pm`, whose claims are `{ approved: true }` — it is deliberately **not** a
global organizer. At 390×844, open the disclosure and assert that this persona sees Events,
Tracker, and account content but does not receive Contacts, Documents, or admin-only links.
Keep its `scrollWidth <= clientWidth` assertion.

Required coverage:

- Pure unit test for every `visibility` × viewer combination and for explicit placements.
- PM/tech narrow menu: Events + Tracker + account only.
- Organizer narrow menu: Events + Tracker + Contacts + Documents + account, no admin group.
...
- Tightening Firestore rules or adding route guards for Contacts/Documents; tracked in
  [IDEAS §5](IDEAS.md).
- Changing the existing Tracker link on the Events screen; it remains consistent with the
  decision to show membership-scoped Tracker to everyone.
